...
from queue_manager import _cleanup_remote_training_staging as cleanup


def _cleanup_remote_training_staging(export_dir):
    target = Path(export_dir)
    parts = target.parts
    if "modal_jobs" not in parts:
        cleanup(export_dir)
        return
    index = parts.index("modal_jobs")
    cleanup(export_dir, staging_root=str(Path(*parts[:index + 1])), job_id=parts[index + 1])
