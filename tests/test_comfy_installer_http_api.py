...
@pytest.mark.asyncio
async def test_image_diagnostic_start_and_archive_routes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = tmp_path / "config.json"
    ...
    monkeypatch.setattr(
        service,
        "start_image_diagnostic",
        lambda: {"state": "running", "operation": "image_diagnostic"},
    )
    monkeypatch.setattr(service, "image_diagnostic_archive", lambda _value: archive)

    client = TestClient(TestServer(app))
    await client.start_server()
    try:
        response = await client.post("/api/comfy-installer/image-diagnostic", json={})
        assert response.status == 200
        assert await response.json() == {
            "ok": True,
            "state": "running",
        ...
        response = await client.get(
            "/api/comfy-installer/image-diagnostic/20260920_120000-1234abcd/archive"
        )
        assert response.status == 200
