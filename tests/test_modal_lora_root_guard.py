...
def test_store_uses_independent_scoped_root(tmp_path: Path) -> None:
    ...
    target = Path(stored[0]["local_path"])
    assert target.is_relative_to(Path(config["instance_lora_load_path"]))
    assert target.read_bytes() == b"lora"
