...
def test_installed_copy_name_resolves_models(tmp_path):
    ...
    # Actual workflow references can add models beyond the declared pack list.
    assert set(item["model_ids"]) <= set(ids)
    assert set(model_ids_for_workflow_files(root, [str(item["filename"])])) <= set(ids)
