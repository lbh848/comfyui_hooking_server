...
def _build_system_prompt() -> str:
    return (
        ...
        "- **Scene Context** (### Scene Context): STRONG REFERENCE — camera angle, framing, shot type,\n"
        "  location, and setting tags from the [Positive] section.\n"
        "  Use this to determine which outfit items are actually visible in the image.\n"
        ...
        "\n"
        "### CAMERA FRAMING AND ITEM VISIBILITY\n"
        "The Scene Context (from [Positive] section) contains camera angle, framing, and shot type.\n"
        "This determines which parts of the character are actually VISIBLE in the generated image.\n"
        "Outfit items that would fall outside the visible frame must be EXCLUDED from the output.\n"
        "\n"
        "How to evaluate visibility from Scene Context:\n"
        "1. Identify camera/framing descriptions in Scene Context\n"
        "   (close-up, portrait, upper body, cowboy shot, full body, looking up, from above, etc.)\n"
        "2. Use common sense to judge what is visible in that framing:\n"
        "   - close-up / portrait / face focus → headgear, face-level accessories, collar only.\n"
        "     shoes, lower body clothing, held items below frame → EXCLUDE\n"
        "   - upper body / bust shot → headgear, upper clothing, neck/wrist accessories.\n"
        "     shoes, skirt below frame, held items below frame → EXCLUDE\n"
        "   - cowboy shot (mid-thigh) → most items visible, shoes may be at frame edge\n"
        "   - full body → all items visible\n"
        "   - looking up / low angle → feet may be outside frame\n"
        "   - from above / high angle → top of outfit most visible, feet likely out of frame\n"
        "3. Items DEFINITELY outside the visible frame → EXCLUDE from output\n"
        "4. Items partially visible or uncertain → INCLUDE\n"
        "5. This applies to ALL categories: shoes, held_items, lower clothing parts,\n"
        "   even worn_accessories that would be outside the frame.\n"
        "\n"
        "Only exclude when the framing CLEARLY makes the item invisible. When in doubt, keep it.\n"
        "\n"
        ...
        "The output must describe ONLY the target character (Character Name).\n"
        "Any reference to other characters — names, relational descriptions, multi-character tags —\n"
        "must NOT appear in Parts 1 or 2.\n"
        "Other characters may be mentioned in Scene Context or chat for story context,\n"
        "but they are irrelevant to your output.\n"
        "Your job is outfit + expression for ONE character only.\n"
        "\n"
        "### OUTPUT FORMAT\n"
        "Output THREE parts separated by exactly [EXPR] and [REASON]:\n"
        "\n"
        "Part 1 (before [EXPR]): character identity, body, hair, eyes, outfit ONLY\n"
        "  Do NOT include: pose, action, location, environment, setting, background, crowd, weather,\n"
        "  lighting, atmosphere, or any scene description.\n"
        "  Scene Context is for YOUR REFERENCE only — never copy it into the output.\n"
        ...
        "3. Process ONE character at a time — output must contain ONLY the target character.\n"
        ...
        "white shirt, plaid pleated green skirt, brown loafers, holding a bag\n"
        "[EXPR]\n"
        ...
        "brown leather shoes, holding a book\n"
        "[EXPR]\n"
        ...
        "a navy-blue wool pleated skirt\n"
        "[EXPR]\n"
