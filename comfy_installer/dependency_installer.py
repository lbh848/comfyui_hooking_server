...
def _wheel_matches_platform(wheel: dict) -> bool:
    ...
    if declared and platform.system() not in {str(value) for value in declared}:
        return False
    ...
    try:
        ...
        print(f"[Python] Cannot inspect wheel platform, deferring to uv: {filename}")
        traceback.print_exc()
        return True
    supported = set(platform_tags()) | {"any"}
    return any(tag.platform in supported for tag in tags)
