...
def sync_models_from_source(
    ...
    선택한 팩의 모델 명세를 호출자가 전달한다. 예전 호출은 이미지의 명세를 사용한다.
    무결성은 해당 명세의 sha256으로 검증하며, 불일치 시 받은
    ...
    # The selected pack can be newer than this deployed image.
    # Legacy callers without metadata still use the image's bundled catalog.
    if model_entries is None:
        manifest_path = Path("/opt/soya/install_manifest.json")
        if not manifest_path.is_file():
            print(f"[MODAL_MODEL_SYNC] Missing manifest: {manifest_path}")
            raise FileNotFoundError(f"Missing manifest: {manifest_path}")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        model_entries = manifest.get("models", [])
    by_id = {str(entry.get("id")): entry for entry in model_entries}
