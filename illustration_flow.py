...
def stage(label):
    ...
    def decorate(fn):
        ...
        async def wrapped(*args, **kwargs):
            ...
            node_id = add_node(run, label, dependencies=_frontier.get(), kind="stage",
                               input=serializable(inputs), status="processing", started_at=time.time())
            try:
                ...
                print(f"[ILLUST_FLOW] 단계 실패: label={label}, error={exc}")
                traceback.print_exc()
                update(run, node_id, status="cancelled" if isinstance(exc, asyncio.CancelledError) else "failed", error=str(exc))
                raise
            finally:
...
def queue_added(item):
    ...
    node_id = add_node(run, item.label, dependencies=() if is_root else _frontier.get(),
                       node_id=item.id, kind="request" if is_root else "image",
...
def queue_sync(items):
    for item in items:
        ...
        if node["status"] not in TERMINAL and status in TERMINAL:
            update(run, node_id, status=status, error=str(getattr(item, "error", "") or ""))
...
def queue_execution(fn):
    ...
    async def wrapped(self, item):
        ...
        update(run, node_id, status="processing", summary="큐에서 실행 시작")
        if root:
        ...
        try:
            ...
            if root:
                ...
                add_node(run, "결과 반환", dependencies=leaves, status=status,
                         ended_at=time.time(), kind="result", output=serializable(result))
                run["status"] = status
                changed(run)
            return result
        except BaseException as exc:
            status = "cancelled" if isinstance(exc, asyncio.CancelledError) else "failed"
            print(f"[ILLUST_FLOW] 실행 실패: run={run['id']}, node={node_id}, error={exc}")
            traceback.print_exc()
            update(run, node_id, status=status, error=str(exc))
            if root:
                run["status"] = status
                changed(run)
            raise
...
def llm_call(fn):
    ...
    async def wrapped(call_name, messages, *args, **kwargs):
        ...
        if node_id is not None:
            _reserved.set(None)
            update(run, node_id, label=call_name, input=messages)
        else:
            node_id = add_node(run, call_name, dependencies=_frontier.get(), kind="llm", input=messages)
        ...
        try:
            ...
            print(f"[ILLUST_FLOW] LLM 종료: call={call_name}, error={exc}")
            traceback.print_exc()
            update(run, node_id, status="cancelled" if isinstance(exc, asyncio.CancelledError) else "failed", error=str(exc))
            raise
        finally:
...
def create_task(coro, *, flow_label=None, **kwargs):
    ...
    ends = []
    run = _run.get()
    reserved = add_node(run, flow_label, dependencies=_frontier.get(), kind="llm") if flow_label else None
    started = False
    async def tracked():
        nonlocal started
        started = True
        ...
        try:
            ...
            if reserved and run["nodes"][reserved]["status"] == "waiting":
                ...
                update(run, reserved, status="cancelled", error="호출 전 작업 종료")
    ...
    def on_done(done):
        ...
        if reserved and done.cancelled() and run["nodes"][reserved]["status"] not in TERMINAL:
            ...
            update(run, reserved, status="cancelled", error="대기 작업 취소")
