...
def _cleanup_remote_training_staging(export_dir: str, *, staging_root: str = "", job_id: str = "") -> None:
    """Remove only this remote job's exported inputs, within the caller's staging root."""
    if not export_dir or not staging_root or not job_id:
        print(f"[QUEUE-TRAIN] Staging cleanup skipped: no remote job ownership, path={export_dir!r}")
        return
    try:
        normalized = os.path.abspath(export_dir)
        boundary = os.path.abspath(staging_root)
        job_root = os.path.abspath(os.path.join(boundary, job_id))
        if (os.path.dirname(job_root) != boundary
                or os.path.realpath(normalized) != normalized
                or os.path.realpath(boundary) != boundary
                or os.path.commonpath((job_root, normalized)) != job_root):
            print(f"[QUEUE-TRAIN] Staging cleanup path rejected: path={normalized}, root={job_root}")
            return
        if not os.path.isdir(normalized):
            print(f"[QUEUE-TRAIN] Staging cleanup skipped: missing folder {normalized}")
            return
        shutil.rmtree(normalized)
        print(f"[QUEUE-TRAIN] Staging cleanup completed: {normalized}")
        parent = os.path.dirname(normalized)
        while os.path.commonpath((boundary, parent)) == boundary and os.path.isdir(parent):
            if os.listdir(parent):
                print(f"[QUEUE-TRAIN] Parent cleanup stopped: folder still in use {parent}")
                break
            os.rmdir(parent)
            if parent == boundary:
                break
        ...
        print(f"[QUEUE-TRAIN] Staging cleanup failed: path={export_dir}, error={exc}")
        traceback.print_exc()
...
LLM_TYPES = frozenset({
    ...
    "illustration_quality_inspection", # 생성 직후 개별 검사 및 완성 세트 전체 품질 검사
...
class QueueManager:
    ...
    async def _handle_video_instruction_refine(self, item: QueueItem) -> dict:
        """Expand the user's brief direction in the LLM lane without enqueueing render."""

        if self.video_mode is None:
            print(
                "[QUEUE:VIDEO_REFINE] 실행 실패: VideoMode 미주입 "
                f"item={item.id}, params={item.params!r}"
            )
            raise RuntimeError("영상화 모드가 큐에 주입되지 않았습니다")
        params = dict(item.params or {})
        ...
        try:
            await self._notify_progress(
                item,
                {"percentage": 5, "phase": "building_video_instruction_refine"},
            )
            result = await self.video_mode.build_instruction_refine(
                params,
                queue_item_id=item.id,
            )
            if not isinstance(result, dict) or not result.get("draft"):
                print(
                    "[QUEUE:VIDEO_REFINE] 결과 형식 오류: "
                    f"item={item.id}, mode={mode!r}, result={result!r}"
                )
                raise RuntimeError("AI 연출 입력 다듬기 결과 형식이 올바르지 않습니다")
            await self._notify_progress(
                item,
                {"percentage": 100, "phase": "video_instruction_refine_completed"},
            )
            print(
                "[QUEUE:VIDEO_REFINE] 생성 완료: "
                f"item={item.id}, mode={mode}, length={len(str(result['draft']))}"
            )
            return result
        except Exception as exc:
    ...
    async def _handle_asset_lora_training(self, item: QueueItem) -> dict:
        ...
        try:
            ...
            _cleanup_remote_training_staging(
                export_result["target_dir"],
                staging_root=os.path.join(comfy_input_dir, base_folder, "modal_jobs"),
                job_id=item.id if CURRENT_COMFY_EXECUTION_TARGET.get() in REMOTE_COMFY_TARGETS else "",
            )
