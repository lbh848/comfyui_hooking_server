...
DEFAULT_BLOCK_TAG_RULES: tuple[str, ...] = (
    "* hair",
    "* eyes",
    "* breasts",
    "bare shoulders",
...
def list_trained_steps(lora_load_path: str, character: str, entry: str, session: str) -> list:
    ...
    for fname in sorted(os.listdir(session_dir)):
        ...
        if fname.endswith('.metadata.json'):
            print(f"[LORA_TRAINED] 보조 메타데이터 제외: {os.path.join(session_dir, fname)}")
            continue
...
def save_training_prompt_positive_only(character: str, entry: str, filename: str, positive: str) -> dict:
    """LLM 정제(일괄) 결과로 positive만 교체. negative/original_*는 기존값을 그대로 유지한다."""
    if ".." in filename or os.path.sep in filename:
        return {"success": False, "error": "잘못된 파일명"}
    t_dir = _training_dir(character, entry)
    base = os.path.splitext(filename)[0]
    ...
    try:
        ...
        with open(prompt_path, "w", encoding="utf-8") as f:
            json.dump(existing, f, ensure_ascii=False, indent=2)
        print(f"[LORA] 학습 프롬프트 정제(positive-only) 저장 완료: {prompt_path}")
        return {"success": True}
    except Exception as e:
        print(f"[LORA] 학습 프롬프트 정제(positive-only) 저장 실패: {prompt_path} - {e}")
        traceback.print_exc()
        return {"success": False, "error": str(e)}
...
def save_test_prompt_positive_only(character: str, entry: str, filename: str, positive: str) -> dict:
    ...
    try:
        ...
        with open(prompt_path, "w", encoding="utf-8") as f:
            json.dump(existing, f, ensure_ascii=False, indent=2)
        print(f"[LORA] 테스트 프롬프트 정제(positive-only) 저장 완료: {prompt_path}")
        return {"success": True}
    except Exception as e:
        print(f"[LORA] 테스트 프롬프트 정제(positive-only) 저장 실패: {prompt_path} - {e}")
        traceback.print_exc()
        return {"success": False, "error": str(e)}
...
def _new_lora_manage() -> dict:
    """새 설치용 LoRA 관리 데이터 반환."""
    return {
        "loras": {},
        "block_tag_rules": list(DEFAULT_BLOCK_TAG_RULES),
    }
    ...
    if os.path.isfile(LORA_MANAGE_FILE):
        try:
            with open(LORA_MANAGE_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            if "block_tag_rules" not in data:
                print(
                    "[LORA_MANAGE] 블록 태그 규칙 미설정, "
                    f"기본값 {len(DEFAULT_BLOCK_TAG_RULES)}개 적용"
                )
                data["block_tag_rules"] = list(DEFAULT_BLOCK_TAG_RULES)
            ...
            traceback.print_exc()
            print(
                "[LORA_MANAGE] 로드 실패로 기본 블록 태그 규칙 적용: "
                f"{len(DEFAULT_BLOCK_TAG_RULES)}개"
            )
            return _new_lora_manage()
    print(
        f"[LORA_MANAGE] 파일 없음, 기본 블록 태그 규칙 적용: "
        f"{LORA_MANAGE_FILE} ({len(DEFAULT_BLOCK_TAG_RULES)}개)"
    )
    return _new_lora_manage()
