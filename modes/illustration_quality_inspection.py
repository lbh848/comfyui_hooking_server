...
Write short, concrete, issue-focused English. Report a material problem when the primary action or interaction is unclear, actor and receiver cannot be distinguished, required local contact does not read, story-critical orientation or support is lost, gross anatomy is impossible or detached, an unsupported occluder or garment changes coverage, or the scene contradicts the source. Respect natural crop and occlusion. Do not evaluate finger count or fine hand rendering; a fused or detached limb or crop-edge shape that makes the pose unreadable is gross anatomy, not a fine-hand critique.

Judge the visible result without using prose to explain missing pixels. A complete subject reaction, pose, gaze, or aftermath may work with its cause off-frame. Omission of an anonymous partner fragment is not itself a defect when the still remains clear. Conversely, a present fragment fails when it is vague, detached, awkward, or does not clarify the local relation. Do not demand or propose a complete partner, identifiable face, silhouette, or second-person count tag.
