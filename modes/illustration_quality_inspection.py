...
Judge the visible result without using prose to explain missing pixels. A complete subject reaction, pose, gaze, or aftermath may work with its cause off-frame only when that reaction or aftermath is itself the selected fact. When the selected fact is an ongoing interaction, its required contact must be visibly present; a prompt saying `implied`, an unseen edge, or a reaction alone does not supply pixels. The named subject's face may remain the primary focus while one contact is secondary in the same connected composition; do not demand a contact-only insert. Omission of an unnecessary anonymous fragment is not itself a defect, while a required or present fragment fails when it is absent, vague, detached, awkward, or does not clarify the local relation. A broad wall of skin, an unexplained crop-edge body mass, or disconnected body fragments entering from different edges is a material failure when the local actor, receiver, and contact cannot be read from the pixels. Do not demand or propose a complete partner, identifiable face, silhouette, or second-person count tag. Do not penalize a minor literal pose or contact mismatch when the visible relation is natural, semantically equivalent, and preserves the primary story fact; still report reversed roles, missing required contact, or an unsupported state change.
...
_OVERALL_SYSTEM_PROMPT = """Judge a labeled contact sheet only for set-level outfit, identity, chronology, and story-state consistency, plus meaningful named-subject coverage. The source narrative and resolved wardrobe timeline are authoritative. Allow supported changes, but do not invent a transition merely because adjacent images or descriptors differ; report unexplained disappearance, reappearance, or alternation. Report complete omission of a named subject only when the source gives that subject at least one distinct, independently readable current beat and the set instead spends slots on duplicate or weaker moments of another subject. Do not demand equal counts. A name or subject authority alone, dialogue, an off-frame cause, or an unreadable candidate does not require a scene.
